...
class GaladrielConnection(BaseConnection):
    ...
    def _get_client(self) -> OpenAI:
        ...
        if not self._client:
            api_key = self.api_key or os.getenv("GALADRIEL_API_KEY")
            if not api_key:
                raise GaladrielConfigurationError(
                    "Galadriel API key not found in environment"
                )
            ...
            self._client = OpenAI(
                api_key=api_key, base_url=API_BASE_URL, default_headers=headers
            )
        return self._client
    ...
    def generate_strategy_action(
        ...
        **kwargs,
        ...
        """Generate strategy action using Galadriel models"""
        try:
            client = self._get_client()
