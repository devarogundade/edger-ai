...
from src.constants.abi import ERC20_ABI, STRATEGY_ABI
...
class SonicConnection(BaseConnection):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.SONIC_COIN = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE"
    ...
    def register_actions(self) -> None:
        self.actions = {
            ...
            "strategy": Action(
                ...
                parameters=[
                    ActionParameter(
                        "strategy_address", True, str, "Strategy contract address"
                    ),
                    ActionParameter("action", True, int, "Enum of action call type"),
                    ActionParameter("data", True, str, "Bytes of action data"),
                ],
                description="Execute a strategy action",
            ),
        }
    ...
    def _get_swap_route(self, token_in: str, token_out: str, amount_in: float) -> Dict:
        ...
        try:
            ...
            if token_in.lower() == self.SONIC_COIN.lower():
    ...
    def swap(
        ...
        try:
            ...
            current_balance = self.get_balance(
                address=account.address,
                token_address=(
                    None if token_in.lower() == self.SONIC_COIN.lower() else token_in
                ),
            )
    ...
    def strategy(self, strategy_address: str, action: int, data: str) -> str:
        ...
        try:
            ...
            tx = contract.functions.executeCall(params).build_transaction(
                {
                    "from": account.address,
                    "nonce": self._web3.eth.get_transaction_count(account.address),
                    "gasPrice": self._web3.eth.gas_price,
