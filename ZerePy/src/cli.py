...
class ZerePyCLI:
    ...
    def _initialize_commands(self) -> None:
        ...
        # Strategy Chat command
        self._register_command(
            Command(
                name="strategy-chat",
                description="Start a strategy chat session with the current agent",
                tips=["Use 'exit' to end the strategy chat session"],
                handler=self.strategy_chat_session,
                aliases=["strategy_talk"],
            )
        )
    ...
    async def _load_agent_from_database(self, strategy_address):
        try:
            self.agent = ZerePyAgent()
            await self.agent.initFromDatabase(strategy_address=strategy_address)
    ...
    def _load_agent_from_file(self, agent_name):
        try:
            self.agent = ZerePyAgent()
            self.agent.initFromFile(agent_name=agent_name)
            logger.info(f"\n✅ Successfully loaded agent: {self.agent.name}")
    ...
    def _load_default_agent(self, database: bool = False) -> None:
        """Load users default agent"""
        agent_general_config_path = Path("agents") / "general.json"
        file = None
        try:
            if database:
                logger.info("No default agent on database")
            else:
                file = open(agent_general_config_path, "r")
                data = json.load(file)
                if not data.get("default_agent"):
                    logger.error(
                        "No default agent defined, please set one in general.json"
                    )
                    return
    ...
    async def load_agent(self, input_list: List[str], database=False) -> None:
        ...
        if database:
            await self._load_agent_from_database(strategy_address=input_list[1])
        else:
    ...
    def strategy_chat_session(self, input_list: List[str]) -> None:
        """Handle chat command"""
        if self.agent is None:
            logger.info("No agent loaded. Use 'load-agent' first.")
            return
        ...
        logger.info(f"\nStarting strategy chat with {self.agent.name}")
        print_h_bar()
        ...
        while True:
            try:
                ...
                response = self.agent.prompt_llm_strategy(user_input)
                logger.info(f"\n{self.agent.name}: {response}")
                print_h_bar()
