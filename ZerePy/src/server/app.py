...
class ZerePyServer:
    ...
    def setup_routes(self):
        ...
        async def load_agent(name: str, load_request: LoadRequest):
            """Load a specific agent"""
            try:
                if load_request.database:
                    await self.state.cli._load_agent_from_database(
                        strategy_address=name
                    )
                else:
                    self.state.cli._load_agent_from_file(agent_name=name)
