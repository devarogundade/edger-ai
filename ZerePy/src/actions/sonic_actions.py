...
def swap_sonic(agent, **kwargs):
    ...
    try:
        ...
        agent.connection_manager.connections["sonic"].swap(
            token_in=token_in, token_out=token_out, amount=amount, slippage=slippage
        )
        return
...
@register_action("execute-strategy")
def execute_strategy(agent, **kwargs):
    """Execute strategy on Sonic chain.
    This is a passthrough to sonic_connection.strategy().
    """

    try:
        strategy_address = kwargs.get("strategy_address")
        action = kwargs.get("action")
        data = kwargs.get("data")

        # Direct passthrough to connection method
        agent.connection_manager.connections["sonic"].strategy(
            strategy_address=strategy_address, action=action, data=data
        )
        return
